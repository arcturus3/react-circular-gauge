  ...
  return (
    <div
      ...
      style={{
        ...
        backgroundColor: props.mode === 'light' ? '#e6e6e6' : '#1a1a1a',
        userSelect: 'none',
        cursor: 'pointer',
        ...
        <Example
          ...
          gaugeProps={{
            ...
            renderValue: ({ value, fmtValue, rawValue }) => {
              const minValue = -50
              const maxValue = 50
              if (rawValue < minValue && value === minValue) return '<' + fmtValue
              if (rawValue > maxValue && value === maxValue) return '>' + fmtValue
              return fmtValue
            },
            renderTopLabel: ({ value }) => (value >= 0 ? 'hot' : 'cold'),
            renderBottomLabel: '°C',
            arcColor: ({ normValue }) =>
              chroma.scale(['#140b34', '#84206b', '#e55c30', '#f6d746']).correctLightness()(normValue).css(),
            trackColor: '#000000',
            topLabelStyle: {
              fontSize: '35%',
        ...
        <Example
          ...
          gaugeProps={{
            renderContent: ({ normValue }) => (
              ...
              >
                <animated.div
                  style={{ width: '50%', height: '50%', transform: normValue.to((value) => `rotate(${value}turn)`) }}
                >
